...
statementP = wsP (assignP <|> ifP <|> whileP <|> emptyP <|> repeatP <|> returnP <|> exitP <|> fCallSt <|> fDefSt)
  where
    ...
    fDefSt = liftA2 Assign (afterP "function" (Name <$> nameP)) $ DefExp <$> (Def <$> parens (P.sepBy nameP (wsP (P.char ','))) <*> (blockP <* stringP "end"))
